...
def populate(input):
    ...
    for ns in sorted(namespaces.keys(),order_long_to_short):
        ...
        clses = [cls for cls in classes if cls.startswith(ns) or cls.startswith(ns.lower())]
        ...
        classes.difference_update(clses)
        ...
        for src in sorted(clses):
            place(src,os.path.join(path,'Classes',src))
    ...
    for src in classes:
        place(src,os.path.join('Uncategorized','Classes',src))
